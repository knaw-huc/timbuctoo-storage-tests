...
import nl.knaw.huc.timbuctoo.util.StreamIterator;
...
public class DatabaseGetter<KeyT, ValueT> {
  ...
  public Stream<KeyT> getKeys(BdbWrapper.KeyRetriever<KeyT> keyRetriever) {
  ...
  public Stream<ValueT> getValues(BdbWrapper.ValueRetriever<ValueT> valueRetriever) {
    return getItems(() -> Tuple.tuple(keyBinder.entryToObject(key), valueBinder.entryToObject(value)))
      .filter(valueRetriever::filter).map(valueRetriever::get);
  }

  public <U> Stream<U> getKeysAndValues(BdbWrapper.KeyValueConverter<KeyT, ValueT, U> keyValueConverter) {
    return getItems(() -> Tuple.tuple(keyBinder.entryToObject(key), valueBinder.entryToObject(value)))
  ...
  private <U> Stream<U> getItems(Supplier<U> valueMaker) {
    ...
    return StreamIterator.stream(data).onClose(() -> {
      try {
        if (data.cursor != null) {
          data.cursor.close();
